...
A long-running feature branch tends to accumulate dozens of small commits —
work-in-progress saves, fixups, "actually this is how it should work" reversals,
lint cleanups, and so on. By the time the branch is ready to land, the commit
history is more of a working diary than a meaningful narrative. This playbook
codifies a safe, repeatable workflow for collapsing that diary into a small
number of coherent "chapter" commits before merging — without losing any work,
...
It was developed while consolidating `feature/storyline-engine-hash-staleness`
from 46 commits down to 7 with zero conflicts.
...
- Each commit tells a story worth preserving individually (e.g., a series of
  small refactors meant to be reviewed/reverted independently)
- The branch has already been pushed and others are basing work on it (rewriting
  shared history is a separate, higher-stakes problem)
- Commits in the branch heavily interleave (a fix to area A, then a feature in
  B, then another fix to A) and you would need significant reordering to group
  them — see Phase 2 for the trade-off

## Prerequisites

What needs to be in place before starting?

- Clean working tree at the branch tip (no uncommitted changes, no staged
  changes)
- All tests and typechecks passing at HEAD — start from a known-good state
- A clear mental model of the branch's logical chapters (5–10 groups, give or
  take)
- Disk space for backup refs (negligible — git is cheap)
- Willingness to pause and diagnose at any step. This is **not** a one-shot
  command; it is a deliberate sequence with verification gates
...
Create safety nets, plan the groups on paper, draft the final messages, then
build a fresh branch from the merge base by cherry-picking each group's commits
in order and soft-resetting them into a single squashed commit. After all groups
are built, prove tree equivalence against the backup before swapping the
canonical branch ref. Keep backups around until the merged result has shipped
end-to-end.
...
5. **Verify each group's commits are contiguous in chronological order.** If
   ...
   - **Prefer (a) unless the blur is genuinely unacceptable.** This is the
     single most important risk-reducing decision in the whole playbook.

**Validation:**

- [ ] Every commit in `<base>..HEAD` is mapped to exactly one group
- [ ] Every group's commits are contiguous in chronological order
- [ ] Group count is in the 5–10 range (or whatever you've decided makes sense)
...
1. Run the project's typecheck: `bun run typecheck` (or equivalent)
2. Run the test suite: `bunx --bun vitest run` (or the project's test command)
...
4. **Do not** delete `backup/pre-consolidate` (tag) or
   ...
   shipped.
5. **Do not** `git push --force` yet. Wait for human confirmation that the
   consolidated version is what they want.
...
1. Merge or fast-forward into the target branch following project conventions
   (this repo: branch off `develop`, merge to `develop`; the user handles
   `develop` → `main`).
...
- The tree-equivalence diff is the primary correctness check — it is exact, not
  heuristic.
- Typecheck and test suite are secondary checks; they should pass trivially if
  tree equivalence holds.
- A functional smoke test on the merged target branch is the final gate before
  deleting backups.

## Examples

### Example 1: storyline-engine-hash-staleness

**Context:** A long-running branch addressing artifact hash staleness in the
Storyline Engine had accumulated 46 commits — a mix of schema migrations, API
plumbing, UI surfacing, test additions, and small fixes. **Outcome:** Collapsed
to 7 chapter commits with zero conflicts. Cherry-picks ran cleanly because the
groups were contiguous in chronological order and required no reordering.
Tree-equivalence diff was empty on the first attempt; typecheck and tests passed
without modification. **Lessons:** The single biggest factor in conflict-free
execution was accepting minor group-boundary blur instead of reordering for
clean grouping. Pre-writing all seven commit messages to
`/tmp/consolidate-msgs/` made Phase 4 mechanical. **Reference:**
`docs/projects/storyline-engine/workstreams/artifact-hash-staleness/retro.md`
...
- **2026-04-12** — Initial version, extracted from the
  storyline-engine-hash-staleness consolidation
