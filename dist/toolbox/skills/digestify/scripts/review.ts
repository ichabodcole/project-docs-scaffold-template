...
async function main(argv: string[]): Promise<number> {
  ...
  let markdown: string;
  try {
    markdown = await readInput({ file: v.file as string | undefined, reference: v.reference as string | undefined });
  } catch (e: any) {
    if (e?.code === "ENOENT") {
      process.stderr.write(`error: file not found: ${e.path ?? "<unknown>"}\n`);
      return 2;
    }
    throw e;
  }
  ...
  let payload: Payload;
  try {
    payload = buildPayload(markdown, { title: v.title as string, theme, sessionId: sessionId || "__TBD__", timeout });
  } catch (e: any) {
    process.stderr.write(`error: ${e.message}\n`);
    return 2;
  }
  ...
  let resolveDone!: (val: { code: number; data: any }) => void;
  const done = new Promise<{ code: number; data: any }>((res) => { resolveDone = res; });
