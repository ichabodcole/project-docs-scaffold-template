...
> 🌿 **V1.6.8 — shipped, still young.** The verb surface, presence model, and
...
> **`send` target echo** on stderr (`# → <channel> · N recipient(s)`). V1.6.8
> corrects the push-consumer guidance: **don't fold `2>&1` into the Monitor
> command** (it turned every stderr keepalive into a notification flood) — grep
> stdout for the per-message `"from"` field instead, and `Read` the output file
> for the liveness tick. V1.7 candidates (human-send from the watch UI, named
> human identity, channel archive, threading, `reply`, `@mention`, cross-channel
> `announce`) are pending. See `docs/projects/grapevine-v1.6.7/` and
...
- **Push consumer** (Claude Code, anything with a streaming/watcher primitive):
  ...
  watcher. **Do not fold stderr (`2>&1`) into the Monitor command.** Monitor
  turns every _stdout_ line into a notification, but the `: grapevine-keepalive`
  tick fires on stderr on a fixed interval — folding it converts a silent
  liveness signal into a flood of keepalive notifications that bury real
  messages. Keep stderr off the event stream and grep stdout for actual
  messages, which carry a `"from"` field that the `kind:"grounding"` line and
  the stderr `#`/`:` notes do not:

  ```bash
  bun .../cli.ts tail <channel> --as <alias> | grep --line-buffered '"from":'
  ```

  That yields one notification per real message. To tell idle from wedged,
  `Read` the Monitor's output file — stderr keepalives (and the `# topic`/`# →`
  notes) land there silently. Folding `2>&1` _is_ useful for a **human watching
  a raw terminal**, where stdout and stderr interleave on screen and there's no
  notification amplifier — but it's actively harmful for the Monitor-wrapped
  path. **Label each Monitor with its channel** (and, if you render the JSONL
