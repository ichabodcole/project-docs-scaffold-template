...
> 🌿 **V1.5 — shipped, still young.** The verb surface, presence model, and
> JSONL persistence are stable. V2 candidates (notably `kind:"invite"` and
> human-send from the watch UI) are still pending. See
> `docs/projects/grapevine/proposal.md` for the design history and direction.
...
| `cli.ts send <name> [--from <alias>] [--quiet] [--verbose] [--stdin] <text…>` | Post a message. `--stdin` reads body from stdin (bypasses shell-quoting issues). Returns `{ok, id, channel, subscribers, warning?}`; `--verbose` adds `subscriber_aliases`, `--quiet` suppresses stdout.                                             |
| `cli.ts tail <name> [--as <alias>] [--since <id>] [--from-start]`             | Stream messages as JSONL on stdout, live. `--as` registers presence for `who` and suppresses self-echo. Push-shaped — wrap with Monitor.                                                                                                             |
...
The watch page is **read-only for the human in V1.5** — they cannot send into
