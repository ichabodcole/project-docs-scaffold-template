...
describe("applyTaskUpdate", () => {
  ...
  test("drops invalid status quietly", () => {
    const s = freshState();
    applyTaskAdd(s, { id: "a", title: "A", status: "todo" });
    expect(applyTaskUpdate(s, "a", { status: "bogus" as any, title: "B" })).toBe(true);
...
async function spawnServerReady(args: string[] = []): Promise<{ proc: ReturnType<typeof Bun.spawn>; ready: ReadyInfo }> {
...
async function collectStdout(proc: ReturnType<typeof Bun.spawn>, predicate: (m: any) => boolean, maxMs: number): Promise<any[]> {
  const reader = proc.stdout.getReader();
  const dec = new TextDecoder();
  let buf = "";
  const seen: any[] = [];
...
async function collectWsUntilClose(ws: WebSocket): Promise<any[]> {
  const msgs: any[] = [];
  ws.addEventListener("message", (ev) => {
    try { msgs.push(JSON.parse(ev.data)); } catch { /* skip */ }
  });
...
describe("submit broadcast", () => {
  test("submit reaches both host stdio and connected WS clients", async () => {
    ...
    proc.stdin.write(enc.encode(JSON.stringify({
      type: "init", title: "submit-test",
...
describe("input validation from browser", () => {
  test("task.edit with non-string title is rejected silently", async () => {
    ...
    const titleUpdates = msgs.filter((m) => m.type === "task.update" && m.patch?.title !== undefined);
...
describe("bg.ts wrapper", () => {
  ...
  test("forwards appended cmds-file lines to the underlying server", async () => {
    ...
    appendFileSync(meta.cmds_file, JSON.stringify({
      type: "init", title: "via cmds-file",
    ...
    const parsed = JSON.parse(submitLine!);
    expect(parsed.tasks.find((t: any) => t.id === "z")).toBeDefined();
  }, 15000);
});
