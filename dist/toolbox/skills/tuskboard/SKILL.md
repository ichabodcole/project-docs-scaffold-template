...
name: tuskboard
description:
  Tuskboard is a duplex agent↔user task board in the browser. Agent posts
  tasks; user drags between todo/doing/done columns, edits titles inline, adds
  or deletes tasks, or submits to end the session. Two host modes — STATIC
...
`plugins/toolbox/skills/tuskboard/scripts/watch-events.sh`).
...
> `plugins/toolbox/skills/tuskboard/scripts/...` relative to the repo root. An
