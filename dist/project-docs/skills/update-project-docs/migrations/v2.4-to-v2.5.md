...
bash "${SKILL_DIR}/migrations/scripts/migrate-v2.4-to-v2.5.sh" \
...
The script handles: directory creation (`docs/briefs/TEMPLATES/`,
`docs/briefs/_archive/`), file copies (`briefs/README.md`, `BRIEF.template.md`),
documentation cycle string update, docs_version bump to `2.5.0`, and scaffold
cleanup.

### [Agent] 1. Generate v2.5 scaffold locally
...
**3c. Documentation cycle** — _(handled by script)_ The script updates
`Report → Investigation` to `Brief → Investigation`.
...
**3f. Update version** — _(handled by script)_ The script bumps `docs_version`
to `2.5.0`.

### 4. Clean up scaffold — _(handled by script)_
...
- [ ] Ran `migrate-v2.4-to-v2.5.sh` (creates briefs dirs, copies files, updates
      cycle string, bumps version, cleans scaffold)
