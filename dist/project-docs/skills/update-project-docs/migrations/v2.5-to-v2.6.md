...
This migration is fully automated. No agent steps remain after running the
script.
...
bash "${SKILL_DIR}/migrations/scripts/migrate-v2.5-to-v2.6.sh" --dry-run

# Apply all changes
bash "${SKILL_DIR}/migrations/scripts/migrate-v2.5-to-v2.6.sh"
```

The script handles: renames all `archive/` directories to `_archive/` using
`git mv` (with `mv` fallback), updates all `/archive/` path references in
`docs/**/*.md` and root agent context files, and bumps `docs_version` to
`2.6.0`.

That's it — no manual steps remain. Proceed to Verification below.

### Manual steps (if not using the script)

<details>
<summary>Expand if you prefer to run steps manually</summary>
