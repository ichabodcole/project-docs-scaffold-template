...
export const STATUS_MAP: Record<string, Record<string, string>> = {
  ...
  backlog: { open: "open", todo: "open", done: "done", completed: "done", dropped: "dropped" },
  fragment: { open: "open", promoted: "promoted", dropped: "dropped" },
  cycle: { planned: "planned", active: "active", closed: "closed", abandoned: "abandoned" },
  "design-resolution": {
    draft: "draft",
    "under review": "draft",
    resolved: "resolved",
...
const CONTRACT_BASENAMES = new Set(["README.md", "AGENTS.md", "CLAUDE.md", "SCHEMA.md"]);
...
export function lifecycleOf(body: string, type: string): { value: string | null; raw: string | null } {
...
export function dateOf(body: string, abs: string, repoRoot: string): string | null {
  const m = /\*\*(?:Created|Date Started|Date|Added|Last Updated):?\*\*:?\s*(\d{4}-\d{2}-\d{2})/m.exec(
    body,
  );
  ...
  const out = Bun.spawnSync(
    ["git", "log", "--diff-filter=A", "--format=%as", "-1", "--", relative(repoRoot, abs)],
    { cwd: repoRoot },
  );
...
export function stripConsumedMetadata(body: string): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!/^\*\*(Status|Created|Date Started|Date|Added|Last Updated|Last Reviewed|Author|Investigator|Tags):?\*\*/.test(line)) {
      out.push(line);
      continue;
    }
...
export function derive(body: string, abs: string, docsRelative: string, repoRoot: string): Derived | null {
...
function readConfig(repoRoot: string): { config: Config; existed: boolean } {
  ...
  if (!existsSync(path)) return { config: { docsRoot: DEFAULT_DOCS_ROOT, exclude: [] }, existed: false };
  const raw = JSON.parse(readFileSync(path, "utf8")) as Record<string, unknown>;
  const lint = (raw.lint ?? {}) as Record<string, unknown>;
  return {
    config: {
      docsRoot: typeof raw.docsRoot === "string" ? raw.docsRoot : DEFAULT_DOCS_ROOT,
...
function docsRootIsDirty(repoRoot: string, docsRoot: string): boolean {
  const out = Bun.spawnSync(["git", "status", "--porcelain", "--", docsRoot], { cwd: repoRoot });
...
export function main(argv: string[], repoRoot: string): number {
  ...
  if (!existsSync(docsRoot)) {
    fail(`No ${config.docsRoot}/ directory here. Run this from your project root.`);
    return 1;
  }

  step(`Reading ${config.docsRoot}/`);
  if (!existed) {
    ok(`.project-docs.json is absent; assuming docsRoot "${DEFAULT_DOCS_ROOT}"`);
  } else {
  ...
  for (const abs of files) {
    ...
    const next = frontmatterFor(d) + stripConsumedMetadata(body).replace(/^\n+/, "");
  ...
  if (!existed) {
    ...
    if (dryRun) console.log(`  ${C.blue}[DRY RUN]${C.reset} would create it with lint.adopting: true`);
    else {
      writeConfig(repoRoot, config.docsRoot);
      ok("created, with lint.adopting: true so the gate reports rather than blocks");
    }
  }

  step("Summary");
  ok(`${changed.length} document(s) ${dryRun ? "would gain" : "gained"} frontmatter`);
  ok(`${skipped.length} skipped (already marked, or a README, template or contract page)`);

  if (unmapped.length) {
    warn(`${unmapped.length} document(s) had a **Status:** nobody could map — left blank:`);
    for (const [rel, raw] of unmapped) console.log(`      ${rel}  ${C.dim}("${raw}")${C.reset}`);
  }
  if (undated.length) {
    warn(`${undated.length} document(s) had no date and no first commit — set to 1970-01-01:`);
...
if (import.meta.main) process.exit(main(process.argv.slice(2), resolve(process.cwd())));
