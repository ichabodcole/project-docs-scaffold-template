...
library, any camera library, any backend), but implementation guidance references
specific stacks (Expo/React Native for mobile, Tauri/Rust for desktop) with
notes on adapting to other platforms.
...
lock, you'll trigger dozens of duplicate connection attempts. Set a flag on first
scan, and reset it when the user navigates back to the scan screen.
...
function PairingDisplay({ url }: { url: string }) {
  return (
    <div>
      ...
      <p>Or enter manually: <code>{url}</code></p>
    </div>
  );
}
```
...
persistent connection for real-time data sync. See the
`live-websocket-sync` recipe for the full sync pattern.
