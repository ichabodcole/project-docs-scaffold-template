...
import { appendFileSync, writeFileSync, statSync, openSync, readSync, closeSync, unlinkSync } from "node:fs";
...
async function main(argv: string[]): Promise<number> {
  ...
  const writeEventLine = (line: string) => {
    ...
    catch (e: any) { process.stderr.write(`bg: append to events file failed: ${e?.message}\n`); }
  };
    ...
    try {
      while (true) {
        ...
        while ((nl = buf.indexOf("\n")) >= 0) {
          ...
          if (!line) continue;
          let parsedLine: any;
          try { parsedLine = JSON.parse(line); }
          catch { /* not JSON — still echo into events file */ }

          // First-ready triggers file creation and meta emission.
          if (parsedLine?.type === "ready" && !metaEmitted) {
            sessionId = parsedLine.session_id as string;
            ...
            try { writeFileSync(eventsFile, ""); } catch {}
            try { writeFileSync(cmdsFile, ""); } catch {}
            emitMeta(parsedLine.url, parsedLine.port);
          }
          writeEventLine(line);
          if (parsedLine?.type === "closed") serverClosed = true;
        }
        if (done) break;
      }
    } finally {
      try { stdoutReader.releaseLock(); } catch {}
    }
  })();
  ...
  const pollCmds = async () => {
    ...
    while ((nl = cmdBuf.indexOf("\n")) >= 0) {
      ...
      try { server.stdin.write(enc.encode(line + "\n")); }
      catch (e: any) {
        process.stderr.write(`bg: forwarding cmd to server failed: ${e?.message}\n`);
        return; // server is gone
      }
    }
  };

  const cmdsInterval = setInterval(() => { void pollCmds(); }, 250);
