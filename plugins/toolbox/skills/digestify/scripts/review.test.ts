...
describe("buildPayload", () => {
  ...
  test("accepts theme variants", () => {
    for (const theme of ["digestify", "classic", "cthulhu"] as const) {
      const p = buildPayload("::: question id=q1\nQ?\n:::", {
        title: "T", theme, sessionId: "s", timeout: 1,
      });
  ...
  test("is JSON serialisable", () => {
    const p = buildPayload("::: question id=q1\nQ?\n:::", {
      title: "T", theme: "digestify", sessionId: "s", timeout: 1,
    });
  ...
  test("includes session_id and timeout_seconds", () => {
    const p = buildPayload("::: question id=q1\nQ?\n:::", {
      title: "T", theme: "digestify", sessionId: "my-slug", timeout: 900,
    });
...
async function postSubmit(port: number, body: any): Promise<void> {
...
describe("end-to-end via subprocess", () => {
  ...
  test("missing --reference path exits 2", async () => {
    const proc = Bun.spawn({
      cmd: ["bun", "run", SCRIPT, "--no-open", "--reference", "/tmp/does-not-exist-digestify-test.md"],
