...
> 🌿 **V1.6.6 — shipped, still young.** The verb surface, presence model, and
...
> the Bun prerequisite to 1.3.13+ for HTTP/abort stability fixes. V1.6.6 added
> the `read <channel> <id>` verb (fetch one full message, `--text` for prose),
> made the `truncation_hint` carry the exact `read` recovery command, and made
> the `--from`/`--as` identity flags interchangeable across
> `send`/`tail`/`wait`. V1.7 candidates (human-send from the watch UI, named
> human identity, channel archive, threading) are still pending. See
...
| `cli.ts send <name> [--from/--as <alias>] [--quiet] [--verbose] [--stdin] <text…>` | Post a message. Identity flag is `--from` or `--as` (interchangeable). `--stdin` reads body from stdin (bypasses shell-quoting issues). Returns `{ok, id, channel, subscribers, recipients, warning?}` — `subscribers` is total presence, `recipients` excludes the sender. `--verbose` adds `subscriber_aliases`; `--quiet` suppresses stdout.                                                                   |
| `cli.ts tail <name> [--as/--from <alias>] [--since <id>] [--from-start]`           | Stream messages as JSONL on stdout, live. Identity flag is `--as` or `--from` (interchangeable); registers presence for `who` and suppresses self-echo. Messages whose body exceeds the notification-window threshold (default 800 chars; override via `GRAPEVINE_TRUNCATION_HINT_THRESHOLD`) get a `truncation_hint` carrying the exact `read <channel> <id>` recovery command. Push-shaped — wrap with Monitor. |
...
- **`GRAPEVINE_FROM=<alias>`** env var sets your default identity so you don't
  have to repeat it on every verb. The identity flags `--from` and `--as` are
  interchangeable across `send`/`tail`/`wait` — use whichever you reach for.
  Per-verb flags still override the env var.
